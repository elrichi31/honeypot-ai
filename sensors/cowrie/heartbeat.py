...
# Must stay in sync with the Zod defaults in
# apps/ingest-api/src/modules/sensors/sensors.controller.ts
DEFAULT_USERNAMES = ["root", "ubuntu", "admin", "oracle", "postgres", "git",
                     "deploy", "centos", "ansible", "ec2-user", "pi", "user"]
DEFAULT_PASSWORDS = ["HoneyTrap2026!", "AtlasNode91", "CedarRoot88", "DeltaForge73",
                     "EmberStack64", "FalconMesh52", "GraniteKey47", "HarborPulse39",
                     "IronVector28", "JadeMatrix84"]
...
def _generate_userdb(config: dict) -> str:
    # One `user:x:password` line per combination — only these credentials get in.
    # ':' is the field separator, so any value containing it would silently
    # shift the columns and accept the wrong password; drop those instead.
    users = [u for u in config.get("usernames") or [] if u and ":" not in u]
    pwds  = [p for p in config.get("passwords") or [] if p and ":" not in p]
    if not users or not pwds:
        users, pwds = DEFAULT_USERNAMES, DEFAULT_PASSWORDS
    lines = [f"{u}:x:{p}" for u in users for p in pwds]
