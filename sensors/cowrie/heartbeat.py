...
def _apply_config(config: dict, hash_val: str) -> None:
    """Write new cowrie.cfg to signal dir and drop the reload flag."""
    os.makedirs(SIGNAL_DIR, exist_ok=True)
    cfg_content = _generate_cowrie_cfg(config)
    cfg_path    = os.path.join(SIGNAL_DIR, "cowrie.cfg")
    flag_path   = os.path.join(SIGNAL_DIR, "cowrie-reload")

    with open(cfg_path, "w") as f:
        f.write(cfg_content)
    # Write reload flag AFTER config — entrypoint reads them in order
    with open(flag_path, "w") as f:
        f.write("")

    _write_current_hash(hash_val)
    print(f"[beacon] config updated (hash={hash_val}) — cowrie will restart", flush=True)
