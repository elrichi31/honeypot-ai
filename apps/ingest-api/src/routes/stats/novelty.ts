...
export async function noveltyRoute(fastify: FastifyInstance) {
  ...
  fastify.get('/stats/novelty', (request) => {
    ...
    return withCache(fastify.cache, `stats:novelty:${hours}`, NOVELTY_TTL, async () => {
      const windowStart = new Date(Date.now() - hours * 60 * 60 * 1000)
      const [newIps, newCredPairs, newWebPaths, newCommands, topNewIps] = await repo.getNoveltyStats(windowStart)
      return {
        windowHours: hours,
