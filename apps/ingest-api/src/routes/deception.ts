...
import { z } from 'zod'
import { DeceptionService, type Scope } from '../modules/deception/deception.service.js'
...
export async function deceptionRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/deception/overview', (_req, reply) =>
    svc.getOverview(fastify.cache, null, 'deception:overview').then(reply.send.bind(reply)))

  fastify.get('/deception/nodes', (_req, reply) =>
    svc.getNodes(fastify.cache, null, 'deception:nodes').then(reply.send.bind(reply)))
