...
export class ProtocolRepository {
  ...
  async list(protocol: string | null, limit: number, offset: number): Promise<HitRow[]> {
    return this.prismaRead.$queryRaw<HitRow[]>`
      ...
      FROM protocol_hits
      WHERE (${protocol}::text IS NULL OR protocol = ${protocol})
      ORDER BY timestamp DESC
      LIMIT ${limit} OFFSET ${offset}
    `
  }

  async count(protocol: string | null): Promise<number> {
  ...
  async getInsights(protocol: string, isSmb: boolean) {
    const [totals, topIps, topPorts, topUsernames, topPasswords, topCommands, topServices, topDatabases, topDomains, topShares, topNativeOS, topNtlmHashes, eventBreakdown, topCredentials] = await Promise.all([
      this.prismaRead.$queryRaw<Array<{ total: number; unique_ips: number; auth_attempts: number; command_events: number; last_seen: Date | null }>>`
        ...
        FROM protocol_hits WHERE protocol = ${protocol}
      `,
      ...
      this.prismaRead.$queryRaw<Array<{ username: string; count: number }>>`
        ...
        WHERE protocol = ${protocol} AND username IS NOT NULL AND username <> ''
      ...
      this.prismaRead.$queryRaw<Array<{ password: string; count: number }>>`
        ...
        WHERE protocol = ${protocol} AND password IS NOT NULL AND password <> ''
      ...
      this.prismaRead.$queryRaw<Array<{ command: string; count: number }>>`
        ...
        WHERE protocol = ${protocol} AND data ? 'command' AND data->>'command' <> ''
      ...
      this.prismaRead.$queryRaw<Array<{ service: string; count: number }>>`
        ...
        WHERE protocol = ${protocol} AND data ? 'service' AND data->>'service' <> ''
      ...
      this.prismaRead.$queryRaw<Array<{ database: string; count: number }>>`
        ...
        WHERE protocol = ${protocol} AND data ? 'database' AND data->>'database' <> ''
      ...
      isSmb
        ? this.prismaRead.$queryRaw<Array<{ ntlm_hash: string; username: string; count: number }>>`
            ...
            FROM protocol_hits
            WHERE protocol = 'smb' AND data->>'ntlmHash' IS NOT NULL AND data->>'ntlmHash' <> ''
  ...
  async getStats(): Promise<Array<{ protocol: string; count: bigint; last_seen: Date; auth_attempts: bigint }>> {
  ...
  async getPortStats(): Promise<Array<{ protocol: string; dst_port: number; count: bigint; last_seen: Date; auth_attempts: bigint }>> {
    return this.prismaRead.$queryRaw`
