-- Extend threat_ip_summary with the three proof-of-intent signals that already
-- live in this database but were never scored: triggered canary tokens,
-- captured malware samples, and Suricata IDS alerts.
--
-- Why it matters: the risk score was built only from telemetry volume, so an
-- attacker who read a planted .env, replayed the credential, and uploaded a
-- binary scored the same as a scanner that only enumerated paths. See
-- risk-factors.ts#scoreEvidenceFactor.
--
-- malware_samples.src_ip is nullable (some captures have no recorded source)
-- and suricata_alerts.src_ip defaults to '' — both are filtered out so they
-- never create a bogus aggregate row.
...
WITH
  ...
  malware_agg AS (
    SELECT
      src_ip,
      COUNT(*)             AS malware_samples,
      MIN(captured_at)     AS mw_first_seen,
      MAX(captured_at)     AS mw_last_seen
    FROM malware_samples
    WHERE src_ip IS NOT NULL AND src_ip <> ''
    GROUP BY src_ip
  ),
  -- Suricata severity is inverted: 1 is the most severe, so MIN is the worst.
  suricata_agg AS (
    SELECT
      src_ip,
      COUNT(*)             AS suricata_alerts,
      MIN(severity)        AS suricata_worst_severity,
      MIN(timestamp)       AS sur_first_seen,
      MAX(timestamp)       AS sur_last_seen
...
SELECT
  ...
  -- Evidence: proof of intent, not inferred from volume
  COALESCE(mw.malware_samples, 0)    AS malware_samples,
  mw.mw_first_seen,
  mw.mw_last_seen,
  COALESCE(sur.suricata_alerts, 0)   AS suricata_alerts,
  sur.suricata_worst_severity,
  sur.sur_first_seen,
  sur.sur_last_seen,
  -- Derived: earliest / latest across ALL sources
  LEAST(s.ssh_first_seen, w.web_first_seen, p.proto_first_seen, ps.ps_first_seen,
        mw.mw_first_seen, sur.sur_first_seen)                                       AS first_seen,
  GREATEST(s.ssh_last_seen, w.web_last_seen, p.proto_last_seen, ps.ps_last_seen,
           mw.mw_last_seen, sur.sur_last_seen)                                      AS last_seen,
