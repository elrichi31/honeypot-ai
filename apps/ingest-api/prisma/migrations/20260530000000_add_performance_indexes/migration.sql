...
CREATE INDEX IF NOT EXISTS "sessions_session_type_started_at_idx" ON "sessions"("session_type", "started_at" DESC);

-- Events: composite for attack_tags CTE (joins on session_id + filters by event_type)
CREATE INDEX IF NOT EXISTS "events_session_id_event_type_idx" ON "events"("session_id", "event_type");

-- Events: composite for dashboard funnel queries (filter by event_type + success flag)
CREATE INDEX IF NOT EXISTS "events_event_type_success_idx" ON "events"("event_type", "success");

-- ProtocolHits: protocol insights endpoint runs 8 queries filtering by protocol ordered by timestamp
CREATE INDEX IF NOT EXISTS "protocol_hits_protocol_timestamp_idx" ON "protocol_hits"("protocol", "timestamp" DESC);

-- ProtocolHits: per-IP protocol analysis (threat intelligence queries)
CREATE INDEX IF NOT EXISTS "protocol_hits_src_ip_protocol_timestamp_idx" ON "protocol_hits"("src_ip", "protocol", "timestamp" DESC);

-- WebHits: web analytics group by attack_type ordered by timestamp
CREATE INDEX IF NOT EXISTS "web_hits_attack_type_timestamp_idx" ON "web_hits"("attack_type", "timestamp" DESC);
