...
test("int-*: every internal node reports the host LAN IP alongside its public one", () => {
  const yaml = compose(INT_NODES)
  ...
  const ips = yaml.match(/SENSOR_LOCAL_IP: "\$\{HOST_LAN_IP:-\}"/g) ?? []
  assert.ok(nodes.length > 0, "expected at least one int-* node")
  assert.equal(ips.length, nodes.length, "each int-* node must set SENSOR_LOCAL_IP from HOST_LAN_IP")
