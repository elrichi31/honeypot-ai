    ...
    return (
      <tr>
        <td colSpan={6} className="px-4 py-8 text-center text-muted-foreground">
          Sin datos — visitá http://localhost:8080 para generar ataques
        </td>
      </tr>
    )
  }
          ...
          >
            ...
            <td className="px-4 py-3 max-w-[220px]">
              <div className="space-y-0.5">
                {attacker.topPaths.slice(0, 2).map((p, i) => (
                  <p key={i} className="truncate font-mono text-xs text-muted-foreground" title={p}>{p}</p>
                ))}
                {attacker.topPaths.length > 2 && (
                  <p className="text-xs text-muted-foreground/60">+{attacker.topPaths.length - 2} más</p>
                )}
              </div>
            </td>
