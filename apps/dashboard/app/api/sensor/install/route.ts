...
function buildCompose(
  ...
  // Suricata is always included — provides network-level IDS regardless of honeypot selection
  blocks.push([""], SURICATA_BLOCK)

  // If SSH (vector) is not included, add a standalone vector just for Suricata
...
# Sensors: ${services.join(", ")} + Suricata IDS
${clientLine}
...
echo "==> Honeypot sensor installer (${services.join(", ")} + Suricata IDS)"
${sshPortStep}
