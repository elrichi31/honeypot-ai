...
// Proxy to ingest-api's GET /analytics/trends/by-sensor (ANALYTICS_MODULE Fase
// G-backend / H1) — same pattern as app/api/analytics/trends/route.ts. Normal
// tenant scope (not the superadmin-only channel /analytics/comparison uses):
// a client only ever sees a breakdown of their own sensors.
export async function GET(req: NextRequest) {
  ...
  const url = new URL(`${getApiUrl()}/analytics/trends/by-sensor`)
