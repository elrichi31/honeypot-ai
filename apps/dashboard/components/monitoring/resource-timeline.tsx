...
function CpuTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="rounded-lg border border-border bg-card px-3 py-2 text-[11px] shadow-lg">
      ...
      <p className="text-blue-400 font-medium">Load: {payload[0]?.value?.toFixed(2)}</p>
    </div>
  )
}
...
function RamTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="rounded-lg border border-border bg-card px-3 py-2 text-[11px] shadow-lg">
      <p className="text-muted-foreground mb-1">{label}</p>
      <p className="text-emerald-400 font-medium">RAM: {payload[0]?.value?.toFixed(1)}%</p>
      {payload[1] && (
        <p className="text-emerald-300">{payload[1]?.value?.toFixed(2)} GB used</p>
      )}
    </div>
  )
}
            ...
            <ResponsiveContainer width="100%" height={160}>
              <LineChart data={data} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                ...
                <XAxis
                  ...
                  tickLine={false}
                  axisLine={false}
                  interval={Math.max(0, Math.floor(data.length / tickCount) - 1)}
                />
                <YAxis
                  tick={{ fontSize: 10, fill: "hsl(var(--muted-foreground))" }}
                  tickLine={false}
