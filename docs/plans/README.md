...
- [DB_QUERY_PERF.md](DB_QUERY_PERF.md) — auditoría de performance de queries en Postgres con `EXPLAIN ANALYZE` real + tuning de Redis. `protocol_hits` tenía estadísticas del planner rotas (arreglado con `ANALYZE`); índice parcial nuevo para el filtro de deception (99ms→0.6ms, Seq Scan→Index Scan); `maxmemory`+`allkeys-lru` agregado a Redis en los 4 compose files. Implementado y verificado 2026-07-05.
- [ERROR_HANDLING.md](ERROR_HANDLING.md) — auditoría de manejo de errores: dos funciones `apiFetch` distintas con el mismo nombre, mensajes de error del backend descartados antes de llegar al toast, controllers que responden error sin loguearlo primero. Fases 1-4 implementadas (2026-07-05); Fase 5 (request-id de correlación end-to-end) pendiente, opcional.
- [FRONTEND_PERF_UX.md](FRONTEND_PERF_UX.md) — auditoría de feedback visual y performance de carga: `clear()` de los formularios de secretos sin pending/error, 4 rutas sin `loading.tsx`, gráficos de recharts sin code-splitting fuera de la home. Implementado y verificado 2026-07-05.
- [UX_CONSISTENCY.md](UX_CONSISTENCY.md) — auditoría de UX: modal casero en `/users` sin aria-label ni i18n (migrado a Dialog compartido, 2026-07-05), más 4 hallazgos pendientes (confirm() nativo en alerts, falta validación inline, empty states inconsistentes, overflow de tablas sin confirmar en mobile).
