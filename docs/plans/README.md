...
- [DASHBOARD_FIRST_LOAD.md](DASHBOARD_FIRST_LOAD.md) — el dashboard nunca carga a la primera: Fase 0-3 implementadas (2026-07-08: retención 24h + warm-up + semáforo, retry/timeouts/auto-retry en el dashboard, kpi-trends 15→10 queries verificado con EXPLAIN ANALYZE, índice `(sensor_id, started_at)` en `sessions`). Pendiente: solo `REPLICA_CONNECTION_LIMIT`/`pool_timeout` como deuda de observación.
...
- [FRONTEND_AUDIT_2026-07.md](FRONTEND_AUDIT_2026-07.md) — auditoría de frontend (2026-07-13). Casi todo ya cubierto por CLIENT_FETCH_HARDENING / FRONTEND_PERF_UX / DASHBOARD_FIRST_LOAD; sección "already covered" para no rehacer. **Implementado (2026-07-13):** Task 2 (memoización de 3 context providers) y Task 1a (`audit` → server component + searchParams, `/api/audit` borrado como código muerto, gate `requireRole` añadido). Task 1b (`sessions-admin`) **no se convirtió a propósito** (interactive-first, sin paginación/filtros → el patrón no aporta). Deuda menor documentada: `load()` de sessions-admin sin abort, y smoke-test autenticado de audit pendiente.
